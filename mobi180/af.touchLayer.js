...
//    var focusScrolls = $.os.ios; //devices scrolling on focus instead of resizing
    ...
    touchLayer.prototype = {
        ...
        hideAddressBar: function(retry, maxTries) {
            ...
            if ($.os.desktop || $.os.chrome || $.os.kindle) {
...
function init() {
    /*  if (jQuery("textarea:visible,slect:visible,input:visible").length) {
        //打开页面 默认聚焦到第一个输入框
        jQuery("textarea:visible,slect:visible,input:visible")[0].focus();
        //触摸到输入框时立即聚焦
        jQuery("textarea:visible,slect:visible,input:visible").each(function () {
            $(this).tap(function () {
                this.focus();
            }).blur(function () {
                this.blur();
    ...
    if (jQuery("textarea:visible,select:visible,input:visible").length) {
        //打开页面 默认聚焦到第一个输入框
        jQuery("textarea:visible,select:visible,input:visible")[0].focus();

        //触摸到输入框时立即聚焦 失去焦点的时候不触发默认事件
        jQuery("textarea:visible,select:visible,input:visible").each(function () {
            $(this).tap(function () {
                this.focus();
            });
        });
